...
# import fragfoldx
import fragfoldx.tools.plotting as plotting
...
import fragfoldx.tools.pymol_utils as pymol_utils
...
def _assert_cached_query_matches(
    msa_file: Path, input_seq_str: str, seqid: str
) -> None:
    """Verify a cached MSA's query sequence matches the input FASTA sequence.

    The MSA cache is keyed by FASTA header id only (``<msa_cache_dir>/<seqid>.a3m``), so two
    different proteins that share a header would otherwise silently reuse the same MSA. Compare
    the cached query (ungapped/uppercase by construction) to the input sequence, case-insensitively,
    and raise loudly on a mismatch. A corrupt/unreadable cache file is left to crash.
    """
    cached_query = a3mcat.MSAa3m.from_a3m_file(msa_file).query.seq_str
    if cached_query.strip().upper() != input_seq_str.strip().upper():
        raise ValueError(
            f"Cached MSA {msa_file} does not match the input sequence for FASTA header "
            f"'{seqid}' (cached query length {len(cached_query.strip())}, input length "
            f"{len(input_seq_str.strip())}). The MSA cache is keyed by FASTA header, so this is "
            f"almost certainly a header collision: two different sequences share the id "
            f"'{seqid}'.\nFix it by either:\n"
            f"  - removing the stale cached MSA to force a fresh download:\n"
            f"      rm '{msa_file}'\n"
            f"  - or (recommended) renaming the input FASTA header to something unique so the two "
            f"proteins get separate cache entries."
        )
...
def get_colabfold_msa(
    ...
    if not msa_file.exists():
        ...
        _assert_cached_query_matches(msa_file, str(seqs[0].seq), seqid)
        print(
